...
pubDate: 2025-06-20
description: Using event sourcing to implement undo/redo functionality.
showDate: true
...
Current state is derived by replaying events up to the current history index:
...
The key insight I used to implement this is that **undo/redo simply involves moving the history index backward and forward**, while the current state is always reconstructed from the events up to that index.
